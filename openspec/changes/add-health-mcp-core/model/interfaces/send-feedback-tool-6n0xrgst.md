...
An authenticated user; a non-empty message; a source of `user` or `agent`; optionally the name of the tool the feedback concerns.

## Postconditions

The tool SHALL store the feedback with its source, related tool, the sender's user id and the time, and SHALL confirm that it was received.
